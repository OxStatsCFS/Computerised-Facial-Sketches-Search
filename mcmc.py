...
if __name__ == "__main__":
    ...
    parser.add_argument("--nsample", default=20, type=int)
    parser.add_argument("--nwarmup", default=20, type=int)
    parser.add_argument("--nchain", default=2, type=int)
    parser.add_argument("--plothist", default=0, type=int)
    ...
    output = []
    output_path = os.path.join(dir, f'{p}_dim_{mi_estimator}_{ho_model.T}_mcmc.csv')
    ...
    for T in T_to_plot:
        ...
        kernel = NUTS(model, target_accept_prob=0.9, step_size=0.3)        

        mcmc = MCMC(kernel, num_samples=args.nsample, warmup_steps=args.nwarmup, num_chains=args.nchain)
        ...
        if args.plothist != 0:
            if p == 2:
                ...
                sns.histplot(df, x='dim 1', kde=True, ax=axs[0])
                sns.histplot(df, x='dim 2', kde=True, ax=axs[1])
                # fig.suptitle(f'T={T} Example {mi_estimator} Posterior NUTS Samples', size=25)
                plt.savefig(os.path.join(dir, f'{p}_dim_{mi_estimator}_{ho_model.T}.png'))
            elif p == 5:
                fig, axs = plt.subplots(1, 2, figsize=(12,6), dpi=200)
                sns.histplot(df.iloc[:, 0:2], ax=axs[0])
                sns.histplot(df.iloc[:, 2:5], ax=axs[1])
                # fig.suptitle(f'T={T} Example {mi_estimator} Posterior NUTS Samples', size=25)
                plt.savefig(os.path.join(dir, f'{p}_dim_{mi_estimator}_{ho_model.T}.png'))
            elif p == 10:
                fig, axs = plt.subplots(2, 2, figsize=(12,12), dpi=200)
                sns.histplot(df.iloc[:, 0:2], ax=axs[0,0])
                sns.histplot(df.iloc[:, 2:5], ax=axs[0,1])
                sns.histplot(df.iloc[:, 5:8], ax=axs[1,0])
                sns.histplot(df.iloc[:, 8:10], ax=axs[1,1])
                # fig.suptitle(f'T={T} Example {mi_estimator} Posterior NUTS Samples', size=25)
                plt.savefig(os.path.join(dir, f'{p}_dim_{mi_estimator}_{ho_model.T}.png'))
            elif p == 20:
                fig, axs = plt.subplots(2, 2, figsize=(12,12), dpi=200)
                sns.histplot(df.iloc[:, 0:5], ax=axs[0,0])
                sns.histplot(df.iloc[:, 5:10], ax=axs[0,1])
                sns.histplot(df.iloc[:, 10:15], ax=axs[1,0])
                sns.histplot(df.iloc[:, 15:20], ax=axs[1,1])
                # fig.suptitle(f'T={T} Example {mi_estimator} Posterior NUTS Samples', size=25)
                plt.savefig(os.path.join(dir, f'{p}_dim_{mi_estimator}_{ho_model.T}.png'))
